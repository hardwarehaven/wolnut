...
def get_idrac_power_state(host, username, password, verify_ssl):
    ...
    try:
        ...
        logger.warning("Could not get iDRAC power state for %s: %s", host, e)
    return None
...
def power_on_idrac_client(host, username, password, verify_ssl=False, retries=3, delay=5):
    for attempt in range(1, retries + 1):
        ...
        if state == "On":
            logger.info("iDRAC %s already powered on", host)
            return True
        ...
        try:
            ...
            if response.status_code in [200, 202, 204]:
                logger.info("Power ON command sent to iDRAC %s (attempt %d)", host, attempt)
                return True
            else:
                logger.warning("Failed to power on iDRAC %s (attempt %d): %s",
                               host, attempt, response.text)
        except Exception as e:
            logger.warning("iDRAC error on attempt %d for %s: %s", attempt, host, e)

        if attempt < retries:
            logger.info("Retrying iDRAC %s in %s seconds...", host, delay)
            time.sleep(delay)

    logger.error("All attempts failed to power on iDRAC client %s", host)
    return False
